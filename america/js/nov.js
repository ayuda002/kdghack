...
function showClock1() {
    ...
    db.collection("users")
      ...
      .then((snap) => {
        ...
        var nop = size + "人";
        document.getElementById("US").innerHTML = nop;
  　　});
  db.collection("score").doc("lmxHJhtMrs2EBrQvdgc4").get().then((snap) => {
    emotion = snap.data().america_score;
    console.log(emotion)
    document.getElementById("US_emo").innerHTML = emotion;
　　});
db.collection("score").doc("8Joohb3Ci4vZe78amjSw").get().then((snap) => {
  emotion2 = snap.data().japan_score;
  console.log(emotion2)
  document.getElementById("japan_emo").innerHTML = emotion2;
　　});
if(emotion < 0){
  B = 180+(75*emotion);
  console.log(R)
